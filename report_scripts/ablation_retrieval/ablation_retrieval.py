...
models = [
    (
        "full_model",
        f"results/ablation/ablation_contrast{seed}/fp_full",
        f"results/ablation/ablation_contrast{seed}/contrast_full",
    ),
    (
        "no_magma",
        f"results/ablation/ablation_contrast{seed}/fp_magma",
        f"results/ablation/ablation_contrast{seed}/contrast_magma",
    ),
    (
        "no_pairwise_differences",
        f"results/ablation/ablation_contrast{seed}/fp_pairwise",
        f"results/ablation/ablation_contrast{seed}/contrast_pairwise",
    ),
    (
        "no_simulated_data",
        f"results/ablation/ablation_contrast{seed}/fp_simulated",
        f"results/ablation/ablation_contrast{seed}/contrast_simulated",
    ),
    (
        "no_unfolding",
        f"results/ablation/ablation_contrast{seed}/fp_unfolding",
        f"results/ablation/ablation_contrast{seed}/contrast_unfolding",
    ),
]
...
for model_name, fp_dir, contrast_dir in models:
    ...
    print("RUNNING FINGERPRINT RETRIEVAL")
    ...
    print("RUNNING CONTRASTIVE RETRIEVAL")
    contrast_save_dir = Path(contrast_dir) / "out_retrieval"
    contrast_save_dir.mkdir(exist_ok=True)
    # clear folder
    clear_dir_cmd = f"rm {contrast_save_dir / '*'}"
    subprocess.call(clear_dir_cmd, shell=True)
    ...
    merged_ranking = (
        Path(contrast_dir) / "merged_retrieval" / f"merged_ranking_{lam_str}.p"
    )
    ...
    retrieval_results.append(
        (model_name, out_ind_files[-1])
    )  # only contrastive retrieval file
