...
for ckpt in Path(contrast_dir).rglob("best.ckpt"):
...
contrast_file = [
    i
    for i in ensemble_output_dir.glob("retrieval_*.p")
    if "ind_found" not in str(i)
][-1]
fp_file = [i for i in fp_outdir.glob("*.p") if "ind_found" not in str(i)][-1]
